...
fn merge_libs(build_dir: &std::path::Path) -> Result<()> {
    ...
    let mut vcpkg_libs = std::path::PathBuf::from(build_dir);
    vcpkg_libs.extend(["vcpkg_installed", "arm64-osx", "lib"]);
    if !vcpkg_libs.is_dir() {
        panic!("Missing vcpkg lib directory: {}", vcpkg_libs.display());
    }

    let paths = std::fs::read_dir(vcpkg_libs)
        .expect("Error reading vcpkg lib directory.");
