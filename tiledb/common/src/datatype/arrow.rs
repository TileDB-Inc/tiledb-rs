...
pub enum DatatypeToArrowResult {
    ...
    /// use tiledb_common::datatype::Datatype as TileDB;
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult};
    /// assert_eq!(DatatypeToArrowResult::Exact(arrow_schema::DataType::UInt8),
    ///            to_arrow(&TileDB::UInt8, CellValNum::single()));
    /// ```
    ...
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult};
    ///
    /// let arrow = to_arrow(&TileDB::UInt8, CellValNum::try_from(8).unwrap());
    ...
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult};
    ///
    /// let arrow = to_arrow(&TileDB::UInt8, CellValNum::Var);
    ...
    /// mapping to `arrow_schema::DataType::LargeBinary`, which is always variable-length.
    ///
    /// ```
    /// use tiledb_common::array::CellValNum;
    /// use tiledb_common::datatype::Datatype as TileDB;
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult};
    ///
    /// assert_eq!(DatatypeToArrowResult::Exact(arrow_schema::DataType::LargeBinary),
    ///            to_arrow(&TileDB::Blob, CellValNum::Var));
    /// ```
    ...
    /// use tiledb_common::array::CellValNum;
    /// use tiledb_common::datatype::Datatype as TileDB;
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult, ARROW_FIELD_METADATA_KEY_TILEDB_TYPE_HINT};
    ///
    ...
    /// else { unreachable!() };
    /// assert_eq!(Some(TileDB::StringAscii), TileDB::from_str(s).ok());
    /// ```
    Exact(arrow_schema::DataType),
    ...
    /// use tiledb_common::datatype::arrow::{to_arrow, DatatypeToArrowResult};
    ///
    /// assert_eq!(DatatypeToArrowResult::Inexact(arrow_schema::DataType::UInt8),
    ///            to_arrow(&TileDB::StringAscii, CellValNum::single()));
    /// ```
    Inexact(arrow_schema::DataType),
}
...
impl DatatypeToArrowResult {
    ...
    pub fn into_inner(self) -> arrow_schema::DataType {
        match self {
 ...
 */
/// `arrow_schema::Field` metadata key for the original `tiledb_common::datatype::Datatype` variant
/// if there is no exact mapping from `tiledb_common::datatype::Datatype` to `arrow_schema::DataType`.
...
pub enum DatatypeFromArrowResult {
    ...
    /// use tiledb_common::array::CellValNum;
    /// use tiledb_common::datatype::Datatype as TileDB;
    /// use tiledb_common::datatype::arrow::{from_arrow, DatatypeFromArrowResult};
    ///
...
pub fn from_arrow(value: &arrow_schema::DataType) -> DatatypeFromArrowResult {
    use arrow_schema::DataType as ADT;
...
pub mod strategy {
            ...
            move |strategy| {
                prop_oneof![
                    ...
                    proptest::collection::vec(
                        (prop_field_name(), strategy.clone(), any::<bool>()),
    ...
    pub fn prop_arrow_field() -> impl Strategy<Value = Field> {
        (
            prop_field_name(),
            any_datatype(Default::default()),
            any::<bool>(),
...
pub mod tests {
    ...
    fn do_to_arrow_single(tdb_dt: Datatype) {
        ...
        match arrow_dt {
            DatatypeToArrowResult::Inexact(arrow) => {
                ...
                let arrow_size = arrow.primitive_width().unwrap();
                assert_eq!(
                    tdb_dt.size(),
                    arrow_size,
                    "to_arrow({}, {:?}) = {}",
            ...
            DatatypeToArrowResult::Exact(arrow) => {
                match arrow {
                    ADT::FixedSizeList(ref item, fixed_len_out) => {
                        ...
                        {
                            let sub_exact =
                                Datatype::from_str(sub_exact).unwrap();
    ...
    proptest! {
        ...
        #[test]
        fn test_from_arrow(arrow in super::strategy::prop_arrow_field()) {
