...
use arrow::datatypes::{self, DataType, Field, IntervalUnit, Schema, TimeUnit};
    ...
    fn default() -> Self {
        SchemaParameters {
            ...
            field_names: proptest::string::string_regex("[a-zA-Z0-9_]*")
                ...
                .boxed(),
            field_type: prop_datatype(Default::default()).boxed(),
        }
    }
}
...
pub fn prop_schema(params: SchemaParameters) -> impl Strategy<Value = Schema> {
    let strat_num_fields = params.num_fields.clone();
    proptest::collection::vec(prop_field(Rc::new(params)), strat_num_fields)
        .prop_map(Schema::new)
}
...
pub fn prop_datatype(
    ...
    let leaf = prop_oneof![
        ...
        prop_datatype_decimal128(),
        prop_datatype_decimal256(),
    ];
...
pub fn prop_datatype_numeric() -> impl Strategy<Value = DataType> {
    prop_oneof![
        Just(DataType::Int8),
        Just(DataType::Int16),
        Just(DataType::Int32),
...
pub fn prop_datatype_integer() -> impl Strategy<Value = DataType> {
    prop_oneof![
        Just(DataType::Int8),
        Just(DataType::Int16),
        Just(DataType::Int32),
...
fn prop_datatype_decimal128() -> impl Strategy<Value = DataType> {
    (1..=datatypes::DECIMAL128_MAX_PRECISION).prop_flat_map(|precision| {
        (
            Just(precision),
            (0..precision.clamp(0, datatypes::DECIMAL128_MAX_SCALE as u8)),
        )
...
fn prop_datatype_decimal256() -> impl Strategy<Value = DataType> {
    (1..=datatypes::DECIMAL256_MAX_PRECISION).prop_flat_map(|precision| {
        (
            Just(precision),
            (0..precision.clamp(0, datatypes::DECIMAL256_MAX_SCALE as u8)),
        )
            .prop_map(|(precision, scale)| {
...
mod tests {
    ...
    proptest! {
        #[test]
        fn unique_names(schema in prop_schema(Default::default())) {
            do_unique_names(schema);
        }

        #[test]
        fn is_numeric(datatype in prop_datatype_numeric()) {
            assert!(datatype.is_numeric());
        }

        #[test]
        fn is_integer(datatype in prop_datatype_integer()) {
